...
TEST(BasicSial,DISABLED_helloworld){
	TestController controller("helloworld", false, VERBOSE_TEST, "this test should print \"hello world\"", std::cout);
	controller.initSipTables();
	controller.runWorker();
	EXPECT_TRUE(controller.worker_->all_stacks_empty());
}
...
TEST(BasicSial,DISABLED_helloworld1){
	...
	controller.initSipTables();
	controller.runWorker();
	if (attr->global_rank() == 0) {EXPECT_EQ(expected.str(), output.str());}
	EXPECT_TRUE(controller.worker_->all_stacks_empty());
}
...
TEST(BasicSial,DISABLED_where_clause){
...
TEST(BasicSial,DISABLED_ifelse){
...
TEST(BasicSial,DISABLED_loop_over_simple_indices){
 ...
 */
void basic_pardo_test(int max_dims, int lower[], int upper[], bool expect_success = true){
	assert (max_dims <=6);
	for (int num_dims = 1; num_dims <= 6; ++num_dims){
		...
		TestController controller(job, true, VERBOSE_TEST, "This is a test of " + job, std::cout, expect_success);
		controller.initSipTables();
		controller.runWorker();
		double total = controller.worker_->scalar_value("total");
		if(VERBOSE_TEST) {std::cout << "num_iters=" << num_iters << ", total=" << total << std::endl;}
...
TEST(BasicSial,DISABLED_pardo_loop){
	int MAX_DIMS = 6;
...
TEST(BasicSial,DISABLED_pardo_loop_corner_case){
	int MAX_DIMS = 6;
 ...
 */
TEST(BasicSial,DISABLED_pardo_loop_illegal_range){
	int MAX_DIMS = 6;
...
TEST(BasicSial,DISABLED_scalar_ops){
...
TEST(BasicSial,DISABLED_int_ops){
...
TEST(BasicSial,DISABLED_tmp_arrays){
...
TEST(BasicSial,DISABLED_scalar_valued_blocks){
...
TEST(BasicSial,DISABLED_tmp_arrays_2){
...
TEST(BasicSial,DISABLED_transpose_tmp){
...
TEST(BasicSial,DISABLED_contraction_small_test){
...
TEST(BasicSial,DISABLED_sum_op){ //block addition
...
//			}
		...
		sip::Block::BlockPtr d_bptr = controller.worker_->get_block_for_reading(d_bid);
		...
		sip::Block::BlockPtr e_bptr = controller.worker_->get_block_for_reading(e_bid);
...
TEST(BasicSial,DISABLED_static_array_test){ //tests extracting blocks from contiguous array
...
TEST(Sial,DISABLED_local_arrays_wild_fail){
...
TEST(Sial,DISABLED_put_test){
	...
	EXPECT_TRUE(controller.worker_->all_stacks_empty());
		...
		int x_slot = controller.worker_->array_slot(std::string("x"));
		int y_slot = controller.worker_->array_slot(std::string("y"));
		...
		sip::Block::BlockPtr x_bptr = controller.worker_->get_block_for_reading(x_bid);
		sip::Block::dataPtr x_data = x_bptr->get_data();
		sip::Block::BlockPtr y_bptr = controller.worker_->get_block_for_reading(y_bid);
		sip::Block::dataPtr y_data = y_bptr->get_data();


		for (int i=0; i<3*3; i++){
			EXPECT_EQ(1, x_data[i]);
			EXPECT_EQ(2, y_data[i]);
		}

}


TEST(BasicSial,DISABLED_contract_to_scalar){
...
TEST(BasicSial,DISABLED_simple_indices_assignments){
...
TEST(BasicSial,DISABLED_self_multiply_test){
...
TEST(SipInterface,DISABLED_get_int_array_test){
...
TEST(SipInterface,DISABLED_get_scalar_array_test){
...
TEST(SipInterface,DISABLED_get_scratch_array_test){
...
TEST(BasicSial,DISABLED_transpose4d_tmp){
...
TEST(BasicSial,DISABLED_transpose4d_square_tmp){
