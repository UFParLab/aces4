...
//			std::cout << "calling broadcast_static with args " << arg0() << ", " << control_stack_.top() << std::endl;
		...
		case get_op: { //TODO  check this.  Have compiler put block info in instruction?
            ...
            gather_loop_indices();
			sial_ops_.get(id, loop_indices_);
			++pc;
		}
			break;
		case put_accumulate_op: { //put a[..] += b[..]  TODO check documentation
            ...
            gather_loop_indices();
			sial_ops_.put_accumulate(lhs_id, rhs_block, loop_indices_);
			++pc;
		}
			break;
		...
		case put_increment_op: { //put a[...] = 0.0
			...
			double rhs_value = expression_stack_.top();
			expression_stack_.pop();
			sial_ops_.put_increment(lhs_id, rhs_value, loop_indices_);
			++pc;
		}
			break;
...
void Interpreter::loop_start(LoopManager * loop) {
    if (op_table_.opcode(pc) == do_op) {
        loop_stack_.push(1);
        loop_indices_.push_back(index_selectors()[0]);
        loop_indices_.push_back(index_selectors()[0]);
    } else {
        loop_stack_.push(arg1());
        for (int i = 0, num = arg1(); i < num; i ++) {
            loop_indices_.push_back(index_selectors()[i]);
            loop_indices_.push_back(index_selectors()[i]);
        }
    }
	...
	if (loop->update()) { //there is at least one iteration of loop
        ...
        for (int i = 0, num = loop_stack_.top(); i < num; i ++) {
...
void Interpreter::gather_loop_indices() {
    
    for (int i = 0; i < loop_indices_.size(); i += 2) {
        loop_indices_[i+1] = data_manager_.index_value(loop_indices_[i]);
    }
}
