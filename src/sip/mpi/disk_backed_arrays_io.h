...
public:
    ...
    void iread_block_from_disk(const BlockId& bid, ServerBlock::ServerBlockPtr bptr);
    ...
    void iwrite_block_to_disk(const BlockId& bid, const ServerBlock::ServerBlockPtr bptr);
    ...
    unsigned int * get_array_bit_map(int array_id);
...
private:
    ...
    bool     *is_persistent_;                   /**< Indicate whether the array is persistent, one per sip array */
    std::list<BlockId> *my_blocks_;                
    std::string *persistent_array_name_;
    
    std::map<BlockId, int> block_index_;
    std::map<int, unsigned int *> block_bit_map_;
    std::map<int, int>  block_bit_map_size_;
    std::map<int, unsigned long long> block_bit_map_offset_;
    
    std::map<int, int> array_file_block_size_;
    std::map<int, unsigned long long> data_start_offset_;
    std::map<int, int> file_blocks_num_;
    
    static const int min_file_block_size_ = 8192 * 1024;
    
    unsigned long long max_array_size;
