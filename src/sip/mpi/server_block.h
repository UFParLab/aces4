...
public:
	...
	bool update_and_check_consistency(SIPMPIConstants::MessageType_t operation, int worker){
		return block_consistency_.update_and_check_consistency(operation, worker);
	}

	/**
	 * Resets consistency status of block to (NONE, OPEN).
	 */
	void reset_consistency_status () { block_consistency_.reset_consistency_status(); }
...
private:
    ...
    DistributedBlockConsistency block_consistency_;/**< To check that Blocks in GET, PUT, PUT+ are following correct semantics */
