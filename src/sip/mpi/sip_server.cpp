...
SIPServer::SIPServer(SipTables& sip_tables, DataDistribution& data_distribution, SIPMPIAttr& sip_mpi_attr,
		             ServerPersistentArrayManager* persistent_array_manager, ServerTimer& server_timer)
                    : sip_tables_(sip_tables)
                    , data_distribution_(data_distribution)
                    , disk_backed_block_map_(sip_tables, sip_mpi_attr, data_distribution, server_timer, server_interpreter_)
                    , server_interpreter_(sip_tables, disk_backed_block_map_)
...
void SIPServer::run() {
	...
	while (!terminated_) {
        ...
        more_blocks_to_prefetch = server_interpreter_.more_blocks_to_prefetch();
        ...
        while (1) {
            ...
            if (!false && (more_blocks_to_prefetch || more_blocks_to_write)) {
                ...
                if (more_blocks_to_prefetch) {
                    //server_interpreter_.prefetch_block1();
		...
		case SIPMPIConstants::GET:{
			handle_GET(mpi_source, mpi_tag, mpi_count);
		}
			break;
		...
		case SIPMPIConstants::PUT_ACCUMULATE:{
			...
			handle_PUT_ACCUMULATE(mpi_source, mpi_tag, put_accumulate_data_tag, mpi_count);
		}
			break;
		...
		case SIPMPIConstants::PUT_INCREMENT:{
			handle_PUT_INCREMENT(mpi_source, mpi_tag, mpi_count);
		}
			break;
...
void SIPServer::handle_GET(int mpi_source, int get_tag, int mpi_count) {
    const int recv_data_count = mpi_count; //BlockId::MPI_BLOCK_ID_COUNT + 2;
    ...
    //server_interpreter_.handle_incoming_block_request(block_id.array_id(), last_seen_worker_, last_seen_line_);
    
	//DEBUG
//	if(block_id.array_id_==137){std::cout << "get block " << block_id << " line "<< last_seen_line_ << std::endl << std::flush;}

    //std::cout << "Server: " << sip_mpi_attr_.global_rank() << ", Block: " << block_id << ", ";

    //for (int i = BlockId::MPI_BLOCK_ID_COUNT + 2; i < mpi_count; i += 2) {
    //    std::cout << sip_tables_.index_name(recv_buffer[i]) << ": " << recv_buffer[i+1] << " ";
    //}
    //std::cout << std::endl;
...
//	}
    ...
    server_interpreter_.handle_block_request(block_id, last_seen_worker_, last_seen_line_, 
                                             &recv_buffer[BlockId::MPI_BLOCK_ID_COUNT + 2], 
                                             mpi_count - (BlockId::MPI_BLOCK_ID_COUNT + 2));
    
    //server_interpreter_.print_workers_iterations();
}
...
//	if(block_id.array_id_==137){std::cout << "put block " << block_id << " line "<< last_seen_line_ << std::endl << std::flush;}
    ...
    server_interpreter_.handle_put_block_request(block_id, last_seen_worker_, last_seen_line_);
}

void SIPServer::handle_PUT_ACCUMULATE(int mpi_source, int put_accumulate_tag,
		int put_accumulate_data_tag, int mpi_count) {
    const int recv_data_count = mpi_count; //BlockId::MPI_BLOCK_ID_COUNT + 2;
	const int line_num_offset = BlockId::MPI_BLOCK_ID_COUNT;
	const int section_num_offset = line_num_offset + 1;
	int recv_buffer[recv_data_count];
...
//	std::cout << *block << std::endl << std::flush;
    server_interpreter_.handle_put_block_request(message.id_, last_seen_worker_, last_seen_line_);

}
void SIPServer::handle_PUT_INCREMENT(int mpi_source, int put_increment_tag, int mpi_count){
    int recv_buffer[mpi_count];
    
	SialOpsParallel::Put_scalar_op_message_t message;
	MPI_Status status;
	MPI_Recv(recv_buffer, mpi_count, MPI_INT, mpi_source, put_increment_tag,
			MPI_COMM_WORLD, &status);
    
    message = *((SialOpsParallel::Put_scalar_op_message_t *)recv_buffer);
    int message_size = (sizeof(message) / 4 + 1) * 4;
...
//	std::cout << "put_increment message.id_=" << message.id_ << " message.line="<< message.line_ << " message.section="<< message.section_ << " message.value_=" << message.value_ << std::endl << std::flush;
    ...
    server_interpreter_.handle_block_request(message.id_, last_seen_worker_, last_seen_line_, 
                                             &recv_buffer[message_size], mpi_count - message_size);
}
