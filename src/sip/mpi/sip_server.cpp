...
void SIPServer::run(){
	...
	while (!terminated_){
		...
		case SIPMPIData::GET :
			...
			break;
		case SIPMPIData::PUT :
			handle_PUT(mpi_source, mpi_tag, state_.make_mpi_tag_for_PUT_DATA(mpi_tag));
			break;
		case SIPMPIData::PUT_ACCUMULATE:
			handle_PUT_ACCUMULATE(mpi_source, mpi_tag, state_.make_mpi_tag_for_PUT_ACCUMULATE_DATA(mpi_tag));
			break;
...
void SIPServer::handle_GET(int mpi_source, int get_tag){
	...
	SIPMPIUtils::check_err(MPI_Recv(buffer, count, MPI_INT, mpi_source, get_tag, MPI_COMM_WORLD, &status));
	...
	SIPMPIUtils::check_err(MPI_Send(block->get_data(), block->size(), MPI_DOUBLE, mpi_source, get_tag, MPI_COMM_WORLD));
}
...
void SIPServer::handle_PUT(int mpi_source, int put_tag, int put_data_tag){
	...
	SIPMPIUtils::check_err(MPI_Recv(buffer, count, MPI_INT, mpi_source, put_tag, MPI_COMM_WORLD, &status));
	check_int_count(status, count);
	//construct a BlockId object from the message contents and retrieve the block.
	BlockId block_id(buffer);
...
void SIPServer::handle_PUT_ACCUMULATE(int mpi_source, int put_accumulate_tag, int put_accumulate_data_tag){
	...
	SIPMPIUtils::check_err(MPI_Recv(buffer, count, MPI_INT, mpi_source, put_accumulate_tag, MPI_COMM_WORLD, &status));
	check_int_count(status, count);
	...
	MPI_Request request;
	SIPMPIUtils::check_err(MPI_Irecv(temp, block_size, MPI_DOUBLE, mpi_source, put_accumulate_data_tag, MPI_COMM_WORLD, &request));
