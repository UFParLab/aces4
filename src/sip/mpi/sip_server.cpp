...
//	if(block_id.array_id_==137){std::cout << "get block " << block_id << " line "<< last_seen_line_ << std::endl << std::flush;}
	...
	ServerBlock* block = disk_backed_block_map_.get_block_for_reading(block_id, last_seen_line());
...
//	}
	...
	if (!block->update_and_check_consistency(SIPMPIConstants::GET, mpi_source)){
		...
		err_ss << "Incorrect GET block semantics for " << block_id << " at line " << last_seen_line()
...
void SIPServer::handle_PUT_DATA(int mpi_source, int put_data_tag){
	...
	if (!block->update_and_check_consistency(SIPMPIConstants::PUT, mpi_source)){
		std::stringstream err_ss;
		err_ss << "Incorrect PUT block semantics (data race) for " << block_id << " at line " << last_seen_line() << " from worker " << mpi_source << ". Probably a missing sip_barrier";
		sial_check(false,err_ss.str());
	}
...
void SIPServer::handle_PUT_ACCUMULATE_DATA(int mpi_source, int put_accumulate_data_tag){
	...
	if (!block->update_and_check_consistency(SIPMPIConstants::PUT_ACCUMULATE, mpi_source)){
		std::stringstream err_ss;
		err_ss << "Incorrect PUT_ACCUMULATE block semantics for " << block_id << " at line " << last_seen_line() << " from worker " << mpi_source
