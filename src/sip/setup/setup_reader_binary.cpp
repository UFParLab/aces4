...
int SetupReaderBinary::predefined_int(const std::string& name) {
...
double SetupReaderBinary::predefined_scalar(const std::string& name) {
...
PredefContigArray SetupReaderBinary::predefined_contiguous_array(const std::string& name){
...
PredefIntArray SetupReaderBinary::predefined_integer_array(const std::string& name){
...
SetupReaderBinary* SetupReaderBinary::get_empty_reader(){
	char *c = new char[1];
	BinaryInputByteStream bis(c, 1);
	SetupReaderBinary *sr = new SetupReaderBinary(bis, false);
	return sr;
}

SetupReaderBinary::SetupReaderBinary(InputStream & stream) : stream_(stream) {
	read();
}

SetupReaderBinary::SetupReaderBinary(InputStream & stream, bool to_read) : stream_(stream) {
	if (to_read)
		read();
}

SetupReaderBinary::~SetupReaderBinary() {
	for(SetupReader::PredefIntArrayIterator iter = predef_int_arr_.begin(); iter != predef_int_arr_.end(); ++iter){
		SIP_LOG(std::cout<<"From SetupReaderBinary, freeing "<<iter->first<<std::endl);
...
//    }
    for (NamePredefinedContiguousArrayMapIterator iter = name_to_predefined_contiguous_array_map_.begin();
    	...
    	SIP_LOG(std::cout<<"From SetupReaderBinary, freeing "<<iter->first<<std::endl);
    	check(iter->second.second, "attempting to delete NULL block in ~SetupReaderBinary");
...
std::ostream& operator<<(std::ostream& os, const SetupReaderBinary & obj) {
	os << "Sial program list:" << std::endl;
	SetupReaderBinary::SialProgList::const_iterator it;
	for (it = obj.sial_prog_list_.begin(); it != obj.sial_prog_list_.end();
			++it) {
		os << *it << std::endl;
	}
	os << "Predefined int map:" << std::endl;
	SetupReaderBinary::PredefIntMap::const_iterator iti;
	...
	SetupReaderBinary::PredefScalarMap::const_iterator its;
	...
	SetupReaderBinary::SetupSegmentInfoMap::const_iterator itg;
	...
	SetupReaderBinary::NamePredefinedContiguousArrayMap::const_iterator itp = obj.name_to_predefined_contiguous_array_map_.begin();
	...
	SetupReaderBinary::PredefIntArrMap::const_iterator itpi;
	...
	SetupReaderBinary::FileConfigMap::const_iterator itc;
	for (itc = obj.configs_.begin(); itc != obj.configs_.end(); ++itc){
		std::string fileName = itc->first;
		os << "[" << fileName << "]" << "{";
		SetupReaderBinary::KeyValueMap kvMap = itc->second;
		SetupReaderBinary::KeyValueMap::const_iterator itkv;
		itkv = kvMap.begin();
		if (itkv != kvMap.end()){
...
void SetupReaderBinary::read_predefined_ints() {
	int n = stream_.read_int();
	for (int i = 0; i < n; ++i) {
...
void SetupReaderBinary::read_predefined_scalars() {
	int n = stream_.read_int();
	for (int i = 0; i < n; ++i) {
		std::string name = stream_.read_string();
...
void SetupReaderBinary::read_segment_sizes() {
...
void SetupReaderBinary::read_predefined_arrays(){
	int n = stream_.read_int();
	for (int i = 0; i < n; i++) {
		// Name of array
		std::string name = stream_.read_string();
...
void SetupReaderBinary::read_predefined_integer_arrays(){
	int n = stream_.read_int();
	for (int i = 0; i < n; i++) {
		// Name of array
		std::string name = stream_.read_string();
...
void SetupReaderBinary::read_sialfile_configs(){
