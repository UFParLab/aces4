...
static void print_usage(const std::string& program_name) {
	...
	std::cerr << "\t\tprofile database : \"profile.db\"" << std::endl;
	std::cerr << "\t\tNumber of worker : 1" << std::endl;
	std::cerr << "\t\tNumber of servers : 1 (if MPI Build)" << std::endl;
...
Aces4Parameters parse_command_line_parameters(int argc, char* argv[]) {
	...
	const char* optString = "d:j:s:p:w:r:h?";
	int c;
